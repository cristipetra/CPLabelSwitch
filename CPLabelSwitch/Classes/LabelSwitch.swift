...
@IBDesignable public class LabelSwitch: UIView {
    ...
    private func calculateSize() {
        calculatedSize = CGSize(width: 70, height: 30)
    }
    
    private func setupCircle() {
        var diameter = bounds.height - 2 * circlePadding
        diameter = 20
        ...
        stateR.circleFrame = CGRect(origin: CGPoint(x: bounds.width - diameter - circlePadding, y: (bounds.height - diameter) / 2 ),
                                    size: circleSize)
